...
class TestOtomasyonApp(ctk.CTk):
    ...
    def testi_oynat(self, test_adi):
        # YENİ: Kullanıcıya döngü sayısını sor
        ...
        if cevap is None: # Kullanıcı iptal tuşuna bastıysa işlemi durdur
            return
        ...
        lbl_baslik = ctk.CTkLabel(self.oynatma_penceresi, text=f"🚀 {test_adi} Testi Çalışıyor ({tekrar_sayisi} Döngü)", font=("Arial", 16, "bold"))
        ...
        self.canli_log.insert("end", f"[SİSTEM] ADB bağlantısı kuruluyor... (Toplam {tekrar_sayisi} kez çalışacak)\n\n")

        # Döngü sayısını arka plan iş parçacığına parametre olarak gönderiyoruz
    ...
    def arka_planda_oynat(self, test_adi, tekrar_sayisi):
        ...
        try:
            # Veritabanından case'leri DÖNGÜDEN ÖNCE sadece 1 kez çekiyoruz
            conn = sqlite3.connect(self.db_yolu)
            cursor = conn.cursor()
            ...
            # Girilen sayı kadar testi tekrarla
            for dongu in range(1, tekrar_sayisi + 1):
                ...
                # Log dosyası ismine hangi döngü olduğunu (Örn: D1, D2) ekliyoruz
                ...
                # Her döngünün raporunu ayrı ayrı kaydediyoruz. 
                # Eğer kullanıcı sadece 1 döngü seçtiyse ismini değiştirmeden kaydet.
                ...
                ekrana_yaz(f"💾 Döngü {dongu} raporu kaydedildi! ({basarili_adim}/{toplam_adim} Adım Başarılı)\n")
                
                # İki test döngüsü arası ufak bir nefes alma boşluğu
                if dongu < tekrar_sayisi:
                    time.sleep(1.5)

            ekrana_yaz(f"\n🎉 TÜM {tekrar_sayisi} DÖNGÜ BAŞARIYLA BİTİRİLDİ!")
